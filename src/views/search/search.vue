...
export default {
  ...
  async created() {
    ...
    const searchTitle = await this.$route.query.title;
    this.articlesData = await search(searchTitle);
  ...
  methods: {
    ...
    async toDetailArticle(num) {
      await this.$router.push('/detailArticle/' + (this.articlesData[num].id).toString())
    },
